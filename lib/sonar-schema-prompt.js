...
Within the <think> block, you MUST:
  1. Identify at least 3-5 key evidence sources for this patient's condition.
  2. For each source, decide on a simple ID (e.g., S1, S2, S3), its title, URL (if available), and a key snippet.
  3. Briefly outline which parts of the ADPIE plan (e.g., specific interventions, diagnoses) will be supported by which source IDs.
...
{intervention_1_rationale}: Provide the evidence-based rationale found in search. **This rationale MUST include inline citations referencing the 'id' from the 'sourcesData' array (e.g., "To detect early signs of respiratory compromise [S1, S2]"). Use the source IDs planned in your <think> block. Aim for at least 2-3 distinct citations in rationales if multiple sources support the intervention.** Must be filled.
...
{intervention_2_rationale}: Provide rationale. **This rationale MUST include inline citations referencing the 'id' from the 'sourcesData' array (e.g., "To promote lung expansion and prevent atelectasis [S1, S3]"). Use the source IDs planned in your <think> block. Aim for at least 1-2 distinct citations here.** Must be filled.
...
{intervention_3_rationale}: Provide rationale. **This rationale MUST include inline citations referencing the 'id' from the 'sourcesData' array (e.g., "To maintain therapeutic drug levels [S2]"). Use the source IDs planned in your <think> block.** Must be filled.
...
{intervention_4_rationale}: Provide rationale. **This rationale MUST include inline citations referencing the 'id' from the 'sourcesData' array (e.g., "To maintain adequate tissue oxygenation [S1, S4]"). Use the source IDs planned in your <think> block. Aim for at least 1-2 distinct citations here.** Must be filled.
